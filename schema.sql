...
END;
$$ language 'plpgsql';
CREATE TRIGGER "tr_user_updated" BEFORE UPDATE ON "users" FOR EACH ROW EXECUTE PROCEDURE update_users_timestamp();
...
CREATE OR REPLACE FUNCTION insert_section_timestamp()
RETURNS TRIGGER AS $$
BEGIN
   NEW.section_created = cast(extract(epoch from now()) as integer);
   RETURN NEW;
END;
$$ language 'plpgsql';
CREATE TRIGGER "tr_section_created" BEFORE INSERT ON "section" FOR EACH ROW EXECUTE PROCEDURE insert_section_timestamp();

CREATE OR REPLACE FUNCTION insert_users_timestamp()
RETURNS TRIGGER AS $$
BEGIN
   NEW.user_created = cast(extract(epoch from now()) as integer);
   RETURN NEW;
END;
$$ language 'plpgsql';
CREATE TRIGGER "tr_user_created" BEFORE INSERT ON "users" FOR EACH ROW EXECUTE PROCEDURE insert_users_timestamp();

CREATE OR REPLACE FUNCTION insert_category_timestamp()
RETURNS TRIGGER AS $$
BEGIN
   NEW.category_created = cast(extract(epoch from now()) as integer);
   RETURN NEW;
END;
$$ language 'plpgsql';
CREATE TRIGGER "tr_category_created" BEFORE INSERT ON "category" FOR EACH ROW EXECUTE PROCEDURE insert_category_timestamp();

CREATE OR REPLACE FUNCTION insert_post_timestamp()
RETURNS TRIGGER AS $$
BEGIN
   NEW.post_created = cast(extract(epoch from now()) as integer);
   RETURN NEW;
END;
$$ language 'plpgsql';
CREATE TRIGGER "tr_post_created" BEFORE INSERT ON "post" FOR EACH ROW EXECUTE PROCEDURE insert_post_timestamp();

CREATE OR REPLACE FUNCTION insert_tag_timestamp()
RETURNS TRIGGER AS $$
BEGIN
   NEW.tag_created = cast(extract(epoch from now()) as integer);
RETURN NEW;
END;
$$ language 'plpgsql';
CREATE TRIGGER "tr_tag_created" BEFORE INSERT ON "tag" FOR EACH ROW EXECUTE PROCEDURE insert_tag_timestamp();

CREATE OR REPLACE FUNCTION insert_comment_timestamp()
RETURNS TRIGGER AS $$
BEGIN
   NEW.comment_created = cast(extract(epoch from now()) as integer);   RETURN NEW;
END;
$$ language 'plpgsql';
CREATE TRIGGER "tr_comment_created" BEFORE INSERT ON "comment" FOR EACH ROW EXECUTE PROCEDURE insert_comment_timestamp();
